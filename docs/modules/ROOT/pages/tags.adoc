...
Use the `--tags` flag to filter CLI output by tag.
This flag is available on `report` and `suppress` commands.

[source,bash]
----
vulnlog suppress my-app.vl.yaml --reporter trivy --tags container
vulnlog report my-app.vl.yaml --tags build-dep
----
