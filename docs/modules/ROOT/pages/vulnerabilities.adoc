...
Stub entry:: A minimal entry containing only required fields (`id`, `releases`, `packages`, `reports`). Created during quick triage.

Under investigation:: The implicit state when no `verdict` has been set. The team is still analyzing the finding.

Analysed:: The entry has a `verdict`, `analysis`, and any required conditional fields (`severity`, `justification`). Optionally includes a `resolution` if the dependency was updated.
