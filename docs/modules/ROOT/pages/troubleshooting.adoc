...
The value passed to `--as-of` does not match any `id` in the file's `releases` section.
...
. The verdict is `affected` or missing, and the report has no `suppress` block: these findings are only suppressed by explicit decision.
