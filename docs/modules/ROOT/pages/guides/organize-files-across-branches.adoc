= Organize files across branches
:page-aliases: branching-strategy.adoc
...
When the same vulnerability needs to be tracked in multiple files, the xref:reference/cli-copy.adoc[`vulnlog modify copy`] command propagates an entry from one file to others without manual duplication.
...
When using the xref:guides/organize-files-across-branches.adoc#per-branch[per-branch] strategy, https://git-scm.com/docs/git-worktree[Git worktrees] simplify working with multiple branches simultaneously.
