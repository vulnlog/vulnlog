...
| `--as-of <release-id>`
| Suppress as of this release, including all releases up to and including it.
...
.Create suppression as of release 8.1.1 for reporter snyk
[source]
----
vulnlog suppress full-example.vl.yaml --as-of 8.1.1 --reporter snyk
Wrote: /path/to/.snyk
----
...
Generating the suppression file with `--as-of 8.0.0` includes the entry, so CI stays green.
