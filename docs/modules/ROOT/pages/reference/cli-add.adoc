...
The `--verdict`, `--severity`, and `--justification` values are written exactly as given and are not cross-checked, so an inconsistent combination (for example `affected` together with a justification) is accepted here and reported later by xref:reference/cli-validate.adoc[validate].
