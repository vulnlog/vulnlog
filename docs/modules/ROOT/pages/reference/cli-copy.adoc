...
Target files are rewritten in the canonical style, exactly as xref:reference/cli-fmt.adoc[fmt] would format them, so a later `fmt` is a no-op.
...
Copied: 1 entry to target1.vl.yml
Copied: 1 entry to target2.vl.yml
----
...
[source,yaml]
----
include::{examplesdir}/copy/after/target1.vl.yml[lines=14..25]
----
