...
Reports never render `risk acceptable` either.
An accepted risk appears as verdict `affected`, its severity, disposition `wont fix`, and state `accepted`, which is exactly what the file records.
