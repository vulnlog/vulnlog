...
* An entry that no release before the fix was affected by, because nobody ever ran a release carrying it.
An entry listed for `1.0.1` and `1.0.2` and fixed in `1.0.2` does appear, because `1.0.1` shipped with the vulnerability.
...
- xref:reference/gradle-plugin.adoc#_vulnlogchangelogreport[The vulnlogChangelogReport Gradle task]
