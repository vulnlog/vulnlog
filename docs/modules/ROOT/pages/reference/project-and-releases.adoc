...
:page-aliases: project-and-releases.adoc
...
This ordering is used by the CLI to resolve `--as-of` filtering.
Validation warns when two releases carrying `published_at` are declared out of order.
...
| Array of String
| No
| Tags associated with this purl. Must reference tags defined in the `tags` section. See xref:reference/tags.adoc[Tags and Filtering].
|===
