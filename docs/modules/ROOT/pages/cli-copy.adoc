...
Copied entries are written in the canonical style, preserving block scalars (`|`/`>`) from the source so the entry reads the same as the original.
On a value collision the destination's style wins, matching the merge rule below.
...
[source,yaml]
----
include::{examplesdir}/copy/after/target1.vl.yml[lines=14..26]
----
