      ...
      - name: Generate suppression files
        ...
        run: |
          docker run --rm \
            ...
            -w /work \
            ghcr.io/vulnlog/vulnlog:{cli-version} \
...
----
docker run --rm -v "$PWD:/work" ghcr.io/vulnlog/vulnlog:{cli-version} suppress vulnlog.yaml --reporter trivy
----
...
[source,bash,subs="attributes"]
----
docker run --rm --user root -v "$PWD:/work" ghcr.io/vulnlog/vulnlog:{cli-version} suppress vulnlog.yaml
----
