...
class SuppressionBuilder {
    ...
    fun inVersion(block: VersionBuilder.() -> Unit) = with(VersionBuilder()) {
        block()
        inVersion.addAll(this.build())
    }

    fun untilVersion(block: VersionBuilder.() -> Unit) = with(VersionBuilder()) {
        block()
...
class ResolutionBuilder {
    ...
    fun ignore(block: IgnoreBuilder.() -> Unit) = with(IgnoreBuilder()) {
        block()
        ignore = this.build()
    }

    fun suppress(block: SuppressionBuilder.() -> Unit) = with(SuppressionBuilder()) {
        block()
        suppress = this.build()
    }

    fun mitigate(block: MitigationBuilder.() -> Unit) = with(MitigationBuilder()) {
        block()
        mitigate = this.build()
    }
