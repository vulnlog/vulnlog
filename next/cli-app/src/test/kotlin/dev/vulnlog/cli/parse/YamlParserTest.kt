...
class YamlParserTest : FunSpec({
    ...
    context("v1 parsing") {
        ...
        test("v1.2 file parses with correct minor version") {
            ...
            parser.parse(yaml).shouldBeInstanceOf<ParseResult.Ok>()
        ...
        test("release entries are parsed by id") {
            ...
            val releases = parser.parse(yaml).shouldBeInstanceOf<ParseResult.Ok>().content.releases
        ...
        test("vulnerability with CVE id is parsed") {
            ...
            val vulns = parser.parse(yaml).shouldBeInstanceOf<ParseResult.Ok>().content.vulnerabilities
        ...
        test("vulnerability with affected verdict and severity is parsed") {
            ...
            parser.parse(yaml).shouldBeInstanceOf<ParseResult.Ok>()
                .content.vulnerabilities[0].verdict shouldBe Verdict.Affected(Severity.HIGH)
        }
        ...
        test("vulnerability with risk_acceptable verdict and severity is parsed") {
            ...
            parser.parse(yaml).shouldBeInstanceOf<ParseResult.Ok>()
                .content.vulnerabilities[0].verdict shouldBe Verdict.RiskAcceptable(Severity.MEDIUM)
        }
