...
### Added
- Add `fmt` command to format Vulnlog files to a canonical style (stable field order, single-element flow arrays, folded block scalars for long text, minimal type-safe quoting); the same formatting is now applied to entries generated by `modify copy`.
- Add `vulnlogFormat` Gradle task (with `--check`) wrapping the `fmt` command.
