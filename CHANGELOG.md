...
### Added
- Add the `vulnlog report changelog` command and the `vulnlogChangelogReport` Gradle task by @ryru in [#261](https://github.com/vulnlog/vulnlog/pull/261). The report lists the vulnerabilities each release fixed, as text or Markdown, ready to paste into release notes. See [`report changelog`](https://vulnlog.dev/docs/vulnlog/0.17/reference/cli-report-changelog.html).
- Add the `disposition` field to record what you intend to do about an affected vulnerability: `will fix` or `wont fix` by @ryru in [#248](https://github.com/vulnlog/vulnlog/pull/248). Set it in the file or with `vulnlog modify add --disposition`. See [verdicts and justifications](https://vulnlog.dev/docs/vulnlog/0.17/reference/verdicts-and-justifications.html).
- Report `accepted` and `not applicable` as separate states by @ryru in [#259](https://github.com/vulnlog/vulnlog/pull/259). A `wont fix` decision carries live risk and a `not affected` verdict does not, so they no longer share one bucket in the HTML report. See [vulnerability states](https://vulnlog.dev/docs/vulnlog/0.17/concepts/vulnerability-states.html).
- Filter impact reports by `--state`, `--verdict`, and `--disposition`, on the CLI and in the Gradle task by @ryru in [#259](https://github.com/vulnlog/vulnlog/pull/259). The active filters are shown in the report header. See [`report impact`](https://vulnlog.dev/docs/vulnlog/0.17/reference/cli-report-impact.html).
- Warn during validation when a critical vulnerability is marked `wont fix` by @ryru in [#248](https://github.com/vulnlog/vulnlog/pull/248).
- Warn during validation when releases are not declared oldest first by @ryru in [#258](https://github.com/vulnlog/vulnlog/pull/258). Release filters follow the declared order, so an out-of-order file gives misleading results.
- Write a "Generated by Vulnlog. DO NOT EDIT." header into Trivy, Snyk, and Cargo Audit suppression files by @rogkne in [#260](https://github.com/vulnlog/vulnlog/pull/260). Every suppress run overwrites the file, so hand edits were lost silently. Generic JSON output is unchanged, because JSON has no comments.

### Changed
- [**breaking**] Rename `vulnlog report` to `vulnlog report impact` by @ryru in [#257](https://github.com/vulnlog/vulnlog/pull/257). The Gradle task `vulnlogReport` becomes `vulnlogImpactReport`, its settings move into a nested `report { impact { } }` block, and the default output file becomes `build/vulnlog/vulnlog-impact-report.html`. The report itself is unchanged; the name makes room for further report types. See the [Gradle plugin reference](https://vulnlog.dev/docs/vulnlog/0.17/reference/gradle-plugin.html).
- [**breaking**] Replace `--release` with `--as-of` on `report impact` and `suppress` by @ryru in [#258](https://github.com/vulnlog/vulnlog/pull/258). The Gradle setting `release` becomes `asOf`. The behaviour is the same: report the state as of that release.
- [**breaking**] Drop `risk acceptable` from `vulnlog modify add --verdict` by @ryru in [#248](https://github.com/vulnlog/vulnlog/pull/248). Use `--verdict affected --disposition "wont fix"` instead. Files that still carry the old verdict keep parsing.
- Write Trivy and Snyk suppression files with the same YAML writer as every other Vulnlog output by @ryru in [#262](https://github.com/vulnlog/vulnlog/pull/262). The content is identical, but indentation and line wrapping differ, so the first run after upgrading produces a one-time diff.

### Deprecated
- Deprecate `verdict: risk acceptable` in favour of `verdict: affected` with `disposition: wont fix` by @ryru in [#248](https://github.com/vulnlog/vulnlog/pull/248). Existing files keep working until 1.0, `vulnlog validate` warns for each legacy entry, and nothing is migrated automatically. See the [migration guide](https://vulnlog.dev/docs/vulnlog/0.17/upgrade/migrate-risk-acceptable.html).

### Fixed
- Fix `vulnlog modify copy` failing when the target file has no releases yet by @ahfoysal in [#226](https://github.com/vulnlog/vulnlog/pull/226).

### Documentation
- Add a "Write release notes" guide and a reference page for `vulnlog report changelog` by @ryru in [#261](https://github.com/vulnlog/vulnlog/pull/261). See [Write release notes](https://vulnlog.dev/docs/vulnlog/0.17/guides/write-release-notes.html).
- Add a migration guide for the deprecated `risk acceptable` verdict by @ryru in [#248](https://github.com/vulnlog/vulnlog/pull/248).
- Link the list of supported scanners at the top of the page, not only at the bottom by @fsommerfeldt in [#255](https://github.com/vulnlog/vulnlog/pull/255).

### New Contributors
* @fsommerfeldt made their first contribution in [#255](https://github.com/vulnlog/vulnlog/pull/255)
